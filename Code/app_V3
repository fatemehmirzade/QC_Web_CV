...
CLASSIFICATION_SCHEMA = {
    "analytical_dimension": {
        ...
        "values": {
            ...
            #isolation purity is about MS2 precursor isolatio not MS1 scan windows
...
UNIT_NAME_TO_ID = {
    ...
    "degree fahrenheit": "UO:0000195",
    "kelvin": "UO:0000012",
    "nanometer": "UO:0000018",
    "micrometer": "UO:0000017",
    ...
    "picomole": "UO:0000041",
    "nanomole": "UO:0000040",
    "microliter": "UO:0000101",
    "milliliter": "UO:0000098",
    "liter": "UO:0000099",
    "unitless": "UO:0000186",
    "dimensionless": "UO:0000186",
    "none": "UO:0000186",
    "arbitrary unit": "UO:0000186",
    "arbitrary units": "UO:0000186",
    "a.u.": "UO:0000186",
    "au": "UO:0000186",
    "nat": "UO:0000186",
    "nats": "UO:0000186",
...
def _resolve_unit(unit_name):
    ...
    if uid:
        return uid
    lower = unit_name.lower().strip()
    for key, val in UNIT_NAME_TO_ID.items():
        if key.lower() == lower:
            return val
    if lower.endswith("s") and len(lower) > 2:
        singular = lower[:-1]
        for key, val in UNIT_NAME_TO_ID.items():
            if key.lower() == singular:
                return val
    return None
...
def parse_obo(filepath):
    """parse a full OBO file into a list of term dicts"""
...
def _is_qc_metric_term(term_display):
    ...
    tid = term_display.get("id", "")
    if tid.startswith("MS:4000"):
        return True
...
def generate_obo_block(result):
    ...
    obo_lines.append(f"is_a: {vtype_id} ! {vtype}")
    if xsd_type:
        obo_lines.append(
    ...
    for cat_name in categories:
        ...
        if cat_id:
            obo_lines.append(
                f"relationship: has_metric_category {cat_id} ! {cat_name}"
            )
    for rel in relations:
        if rel.get("id"):
            obo_lines.append(
                f"relationship: has_relation {rel['id']}"
                f" ! {rel.get('name', '')}"
            )
    for col in columns:
...
def _build_term_type_annotations(candidate_display_terms):
    ...
    rows = []
    for t in candidate_display_terms:
        kind = "QC_METRIC" if _is_qc_metric_term(t) else "BASE_TERM"
        scope_hint = ""
        if kind == "QC_METRIC":
            cats = t.get("categories", [])
            cats_lower = " ".join(cats).lower()
            if "single spectrum" in cats_lower or t.get("value_type") and "spectrum" in str(t.get("is_a", [])).lower():
                scope_hint = "  (spectrum-level)"
            elif "single run" in cats_lower:
                scope_hint = "  (run-level)"
            elif "multiple runs" in cats_lower:
                scope_hint = "  (multi-run-level)"
        else:
...
def build_system_prompt(candidate_display_terms, total_count, active_count):
    ...
    prompt_parts = [
        ...
        "terms (MS:4000xxx) and the base MS vocabulary terms they reference "
        ...
        "The following is the COMPLETE list of IDs available to you.\n"
        ...
        "  ESI, MALDI, DESI, LC, GC, CE, RT, S/N.\n"
        ...
        #step 3: definition + comment
        "STEP 3 -- GENERATE FORMAL DEFINITION AND COMMENT:\n"
        ...
        "Also generate a suggested_comment string (may be null). The comment\n"
        "should convey non-trivial interpretation guidance, such as:\n"
        ...
        "If no useful comment can be added, set suggested_comment to null.\n",
...
def _validate_returned_ids(result, valid_id_set):
    ...
    if warnings:
        ...
        note = (
            ...
            + "; ".join(warnings)
            + " These IDs were not in the provided candidate set and may "
...
def _enforce_base_term_overlap_cap(result, candidate_display_terms):
    """downgrade duplicate/high overlap with base terms to moderate"""
    base_term_ids = {
...
def _stabilize_relations(result, candidate_display_terms):
    ...
    #build lookup for candidate terms
    ...
    for rel in rels:
        ...
        term_name = term.get("name", rel.get("name", ""))
        if any(term_name.startswith(vp) for vp in vendor_prefixes):
            continue

        if rid in overlap_levels and overlap_levels[rid] == "low":
            continue
...
def render_sidebar(display_terms, active_count, api_key):
    ...
    st.sidebar.markdown("---")
    st.sidebar.write("Source: psi-ms.obo")
...
def render_clarification_ui(result, api_key):
    ...
    if st.button("Re-Analyze"):
        ...
        with st.spinner("Re-analyzing with GPT-5.4 ..."):
            ...
            valid_ids = {
                t["id"]
                for t in st.session_state.get("candidate_terms_raw", [])
            }
            if valid_ids:
                result2 = _validate_returned_ids(result2, valid_ids)
            candidate_display = [
                term_to_display_dict(t)
                for t in st.session_state.get("candidate_terms_raw", [])
            ]
            if candidate_display:
                result2 = _enforce_base_term_overlap_cap(
...
@st.cache_data
def load_metrics():
    obo_path = os.path.join(SCRIPT_DIR, "psi-ms.obo")
    if not os.path.exists(obo_path):
        st.error(
            f"OBO file not found at {obo_path}. "
            "Place psi-ms.obo next to this script."
        )
        return [], []
...
def main():
    ...
    st.write(
        ...
        f"**{len(display_terms)}** existing terms "
        f"({active_count} active, {len(display_terms) - active_count} "
        f"obsolete) parsed from the full **psi-ms.obo** ontology using "
        f"**GPT-5.4 **."
    )
    ...
    if analyze_clicked:
        ...
        with st.spinner("Retrieving relevant terms ..."):
            candidate_raw = retrieve_relevant_terms(
                ...
                top_k=TOP_K_EMBED, hops=GRAPH_HOP_LIMIT,
                max_total=MAX_CANDIDATES,
            )
            candidate_display = [
                term_to_display_dict(t) for t in candidate_raw
            ]
        ...
        with st.spinner("Analyzing with GPT-5.4 ..."):
            ...
            result = _validate_returned_ids(result, valid_ids)

            #enforce base term overlap cap
            result = _enforce_base_term_overlap_cap(
                result, candidate_display
            )
